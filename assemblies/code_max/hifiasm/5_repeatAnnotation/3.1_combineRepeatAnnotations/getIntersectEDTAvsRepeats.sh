...
for i in $(ls at*.edta.annotation.gff3); do
	NAME=$(echo $i | awk -F'.' '{print $1}')
	cat $i | awk '$1 !~/^#/ {print $1}' | sort | uniq > $NAME.sorted.edta.IDs
	cat $NAME.scaffolds.bionano.hifiasm.final.fasta.out.gff | awk '$1 !~/^#/ {print $1}' | sort | uniq > $NAME.sorted.repeatMasker.IDs
	bedtools sort -faidx $NAME.sorted.edta.IDs -i $i > $NAME.edta.sorted.gff
	bedtools sort -faidx $NAME.sorted.repeatMasker.IDs -i $NAME.scaffolds.bionano.hifiasm.final.fasta.out.gff > $NAME.repeatMasker.sorted.gff
