...
for i in $(ls at*.bionano.hifiasm.scaffolds.fasta.mod.EDTA.TEanno.gff3); do
	NAME=$(echo $i | awk -F'.' '{print $1}')
	cat $i | awk '$1 !~/^#/ {print $1}' | sort | uniq > $NAME.sorted.edta.IDs
	cat $NAME.bionano.hifiasm.scaffolds.fasta.out.gff | awk '$1 !~/^#/ {print $1}' | sort | uniq > $NAME.sorted.repeatMasker.IDs
	bedtools sort -faidx $NAME.sorted.edta.IDs -i $i > $NAME.edta.sorted.gff
	bedtools sort -faidx $NAME.sorted.repeatMasker.IDs -i $NAME.bionano.hifiasm.scaffolds.fasta.out.gff > $NAME.repeatMasker.sorted.gff
