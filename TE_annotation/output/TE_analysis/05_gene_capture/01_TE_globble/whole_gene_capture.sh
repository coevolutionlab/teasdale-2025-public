...
# get gene MAX
merge_size_gene=$(cat ${name_geneanno}_gene_stats.tsv | grep Max | cut -f2)
# remove decimals
...
parallel -j ${threads} extract_n_merge_by_size {} ${merge_size_gene} ${TE_anno} "exon_max_size" < TE_Families.txt

mkdir -p tmp_exon_MAX_size/
mv  exon_max_size*_merged.fam.bed tmp_exon_MAX_size/
