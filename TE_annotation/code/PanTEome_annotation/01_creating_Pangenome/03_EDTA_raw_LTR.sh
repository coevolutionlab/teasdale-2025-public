...
# This script runs EDTA_raw  LTR module in  each gneome  with the names already modified separately to  try to speed up the proccess
...
do
    perl ${EDTA_raw} --threads 16 --genome ${genome} --type ltr |&  tee $(basename ${genome} .fasta)_EDTA_raw_ltr.log ;
done
