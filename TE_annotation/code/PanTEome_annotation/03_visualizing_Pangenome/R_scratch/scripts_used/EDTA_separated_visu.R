...
dist1 <- EDTA_18_raw_GFF %>% dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
...
yay1 <- EDTA_18_raw_GFF %>% dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
  ggplot(aes(x=X10,y=log2(length), color=X11)) + 
  geom_boxplot() +
...
perc_identity1 <- EDTA_18_raw_GFF %>% dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
