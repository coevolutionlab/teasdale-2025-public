...
#File:
pangenome_GFF <- vroom("/tmp/global2/acontreras/difflines_annex_adri/TE_annotation/output/FINAL_TE_annotation/pangenome_TEannotation.gff3", 
                           col_names = FALSE, delim = "\t", skip = 8)


# Add TE length
pangenome_GFF <-  pangenome_GFF %>% dplyr::mutate(length=X5-X4)
#Retrieve method
pangenome_GFF <- pangenome_GFF  %>%  
...
pangenome_GFF %>%  filter(!grepl("CEN.*", X3))  %>%  
  dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
...
G1 <-  pangenome_GFF %>%  filter(!grepl("CEN.*", X3))  %>% 
  dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
  filter(!grepl(".*rDNA.*", X3))  %>% 
  filter(!grepl("Satellite", X3))  %>% 
  filter(!grepl("telomere", X3))  %>% 
...
a1 <- pangenome_GFF %>%  dplyr::filter(X11 == "DENOVO") %>% 
  dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
  dplyr::group_by(X10) %>%  
  dplyr::tally() %>%  
...
b2 <- pangenome_GFF %>%  dplyr::filter(Annotation == "DENOVO") %>% 
  dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
...
b2_alternative <- pangenome_GFF %>%  dplyr::filter(Annotation == "DENOVO") %>% 
  dplyr::filter(!grepl(".*Parent.*", X9)) %>% 
  dplyr::filter(classification != "pararetrovirus") %>% 
...
pangenome_GFF %>%  dplyr::filter(X11 == "DENOVO") %>% 
  dplyr::filter(X3 != "long_terminal_repeat") %>% 
  dplyr::filter(X3 != "target_site_duplication") %>% 
  dplyr::filter(classification != "pararetrovirus") %>% 
  dplyr::filter(!grepl("telomere.*|.*CEN|.*rDNA", X3)) %>% 
...
pangenome_GFF <- dplyr::left_join(pangenome_GFF, pangenome_intact_GFF[,c(20,21)], by ="TEfam") %>%  
                        dplyr::mutate(anno = replace_na(anno, "incomplete"))

pangenome_GFF %>%     dplyr::filter(X3 != "long_terminal_repeat") %>% 
  dplyr::filter(X3 != "target_site_duplication") %>% 
  dplyr::filter(X11 != "centromere") %>% 
  dplyr::filter(X11 != "telomere") %>%
  dplyr::filter(X11 != "rDNA") %>%
...
denovo_Helitrons <- pangenome_GFF %>%  filter(X3 == "RC/Helitron") %>%  filter(Annotation == "DeNovo") %>%  filter(X2 != "EAHelitron")
...
a1 <- pangenome_GFF %>%  dplyr::filter(Annotation == "DeNovo") %>% 
  dplyr::filter(X3 != "long_terminal_repeat") %>% 
  dplyr::filter(X3 != "target_site_duplication") %>% 
...
b2 <- pangenome_GFF %>%  dplyr::filter(X11 == "DENOVO") %>% 
  dplyr::filter(X3 != "long_terminal_repeat") %>% 
  dplyr::filter(X3 != "target_site_duplication") %>% 
  dplyr::group_by(X3,X10) %>%  
  dplyr::tally() %>%  
...
b2_alternative <- pangenome_GFF %>%  dplyr::filter(X11 == "DENOVO") %>% 
  dplyr::filter(X3 != "long_terminal_repeat") %>% 
  dplyr::filter(X3 != "target_site_duplication") %>% 
  dplyr::filter(classification != "pararetrovirus") %>% 
  dplyr::group_by(X10,classification) %>%  
...
summary_unknowns <- pangenome_GFF %>%  filter(classification =="Unknown") %>%  
  group_by(TEfam) %>%  
